...
func (this *testClient) Run() {
	...
	for {
		...
		case <-t1.C:
			atomic.AddUint64(&testStat.requestTimeout, 1)
...
func TestSendMsgToTaskOk(t *testing.T) {
	...
	if testStat.requestTimeout != 0 {
		t.Errorf("TestSendMsgToTaskOk failed, wrong requestTimeut = %v", testStat.requestTimeout)
		return
	}
