...
func NewMsgInfo(version, msgId uint32, source, dest TaskId) *MsgInfo {
	return &MsgInfo{
		Version: version,
		MsgId:   msgId,
		Source:  source,
...
func (this *taskPool) ShowTasks() {
	for name, _ := range this.taskNameMap {
		fmt.Printf("Task %s\n", name)
	}
}
...
func SendMsgToTask(msgInfo *MsgInfo, data interface{}, delay time.Duration) (err error) {
	if msgInfo == nil {
...
func ForwardMsg(msg *Message, delay time.Duration) (err error) {
	if msg == nil {
		return errors.New("ForwardMsg: msg is nil")
	}

	item, found := g_taskPool.FindById(&msg.Msginfo.Dest)
	if !found {
		return errors.New(fmt.Sprintf("ForwardMsg: Dest = %#v is not exist", msg.Msginfo.Dest))
	}
